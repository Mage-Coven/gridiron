...
func InitGenesis(
	...
	for i, m := range data.Contracts {
		info := m.ContractInfo
		var d types.GridironContractDetails
		...
		if d.HasRegisteredPrivilege(types.PrivilegeStateExporterImporter) {
			...
			bz, err := json.Marshal(contract.GridironSudoMsg{Import: &model.Msg})
			if err != nil {
...
func ExportGenesis(ctx sdk.Context, keeper *Keeper) *types.GenesisState {
	...
	for i, v := range wasmState.Contracts {
		...
		bz, err := json.Marshal(contract.GridironSudoMsg{Export: &struct{}{}})
		if err != nil {
