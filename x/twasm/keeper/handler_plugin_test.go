...
import (
	...
	"github.com/mage-coven/gridiron/x/twasm/contract"
	"github.com/mage-coven/gridiron/x/twasm/types"
)

func TestGridironHandlesDispatchMsg(t *testing.T) {
	var (
		contractAddr = RandomAddress(t)
		otherAddr    = RandomAddress(t)
	)
	specs := map[string]struct {
		setup                 func(m *handlerGridironKeeperMock)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			mock := handlerGridironKeeperMock{}
			consensusStoreMock := NoopConsensusParamsStoreMock()
			spec.setup(&mock)
			h := NewGridironHandler(cdc, mock, bankMock, consensusStoreMock, govRouter)
...
func TestGridironHandlesPrivilegeMsg(t *testing.T) {
	myContractAddr := RandomAddress(t)

	var capturedDetails *types.GridironContractDetails
	captureContractDetails := func(ctx sdk.Context, contract sdk.AccAddress, details *types.GridironContractDetails) error {
	...
	captureWithMock := func(mutators ...func(*wasmtypes.ContractInfo)) func(mock *handlerGridironKeeperMock) {
		return func(m *handlerGridironKeeperMock) {
	...
	specs := map[string]struct {
		...
		expDetails         *types.GridironContractDetails
		...
		"unregister begin block": {
			...
			setup: captureWithMock(func(info *wasmtypes.ContractInfo) {
				ext := &types.GridironContractDetails{
					RegisteredPrivileges: []types.RegisteredPrivilege{{Position: 1, PrivilegeType: "begin_blocker"}},
				}
				info.SetExtension(ext)
			}),
			expDetails:         &types.GridironContractDetails{RegisteredPrivileges: []types.RegisteredPrivilege{}},
		...
		"register begin block with existing registration": {
			src: contract.PrivilegeMsg{Request: types.PrivilegeTypeBeginBlock},
			setup: captureWithMock(func(info *wasmtypes.ContractInfo) {
				info.SetExtension(&types.GridironContractDetails{
					RegisteredPrivileges: []types.RegisteredPrivilege{{Position: 1, PrivilegeType: "begin_blocker"}},
				})
			}),
		},
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			mock := handlerGridironKeeperMock{}
			spec.setup(&mock)
			h := NewGridironHandler(nil, mock, nil, nil, nil)
			var ctx sdk.Context
...
func TestHandleGovProposalExecution(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			h := NewGridironHandler(cdc, mock, nil, nil, router)
			var ctx sdk.Context
...
func TestHandleMintToken(t *testing.T) {
	...
	specs := map[string]struct {
		...
		setup          func(k *handlerGridironKeeperMock)
		expErr         *sdkerrors.Error
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			keeperMock := handlerGridironKeeperMock{}
			spec.setup(&keeperMock)
			h := NewGridironHandler(cdc, keeperMock, mock, nil, nil)
			var ctx sdk.Context
...
func TestHandleConsensusParamsUpdate(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			keeperMock := handlerGridironKeeperMock{}
			spec.setup(&keeperMock)
			h := NewGridironHandler(cdc, keeperMock, nil, mock, nil)
			var ctx sdk.Context
...
func withPrivilegeRegistered(p types.PrivilegeType) func(k *handlerGridironKeeperMock) {
	return func(k *handlerGridironKeeperMock) {
		k.GetContractInfoFn = func(ctx sdk.Context, contractAddress sdk.AccAddress) *wasmtypes.ContractInfo {
			c := wasmtypes.ContractInfoFixture(func(info *wasmtypes.ContractInfo) {
				info.SetExtension(&types.GridironContractDetails{
...
func setupHandlerKeeperMock(m *handlerGridironKeeperMock, mutators ...func(*wasmtypes.ContractInfo)) {
	m.IsPrivilegedFn = func(ctx sdk.Context, contract sdk.AccAddress) bool {
		return true
	}
	m.GetContractInfoFn = func(ctx sdk.Context, contractAddress sdk.AccAddress) *wasmtypes.ContractInfo {
	...
	m.setContractDetailsFn = func(ctx sdk.Context, contract sdk.AccAddress, details *types.GridironContractDetails) error {
		return nil
	}
}

var _ GridironWasmHandlerKeeper = handlerGridironKeeperMock{}

type handlerGridironKeeperMock struct {
	...
	setContractDetailsFn          func(ctx sdk.Context, contract sdk.AccAddress, details *types.GridironContractDetails) error
	GetContractInfoFn             func(ctx sdk.Context, contractAddress sdk.AccAddress) *wasmtypes.ContractInfo
}

func (m handlerGridironKeeperMock) IsPrivileged(ctx sdk.Context, contract sdk.AccAddress) bool {
...
func (m handlerGridironKeeperMock) appendToPrivilegedContracts(ctx sdk.Context, privilegeType types.PrivilegeType, contractAddress sdk.AccAddress) (uint8, error) {
...
func (m handlerGridironKeeperMock) removePrivilegeRegistration(ctx sdk.Context, privilegeType types.PrivilegeType, pos uint8, contractAddr sdk.AccAddress) bool {
...
func (m handlerGridironKeeperMock) setContractDetails(ctx sdk.Context, contract sdk.AccAddress, details *types.GridironContractDetails) error {
...
func (m handlerGridironKeeperMock) GetContractInfo(ctx sdk.Context, contractAddress sdk.AccAddress) *wasmtypes.ContractInfo {
