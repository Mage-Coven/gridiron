...
import (
	...
	"github.com/mage-coven/gridiron/x/poe/contract"
...
var (
	...
	tg4Mixer []byte
	//go:embed contract/gridiron_valset.wasm
	tgValset []byte
	//go:embed contract/gridiron_trusted_circle.wasm
	tgTrustedCircles []byte
	//go:embed contract/gridiron_oc_proposals.wasm
	tgOCGovProposalsCircles []byte
	//go:embed contract/gridiron_community_pool.wasm
	tgCommunityPool []byte
	//go:embed contract/gridiron_validator_voting.wasm
	tgValidatorVoting []byte
	//go:embed contract/gridiron_ap_voting.wasm
	tgArbiterPool []byte
...
// See https://github.com/mage-coven/gridiron-contracts/blob/main/docs/Architecture.md#multi-level-governance for an overview
...
func VerifyPoEContracts(ctx sdk.Context, tk twasmKeeper, poeKeeper keeper.ContractSource) error {
	...
	types.IteratePoEContractTypes(func(tp types.PoEContractType) bool {
		...
		if !tk.IsPinnedCode(ctx, c.CodeID) {
			...
			// fail when https://github.com/mage-coven/gridiron/issues/402 is implemented
