...
HTTPS_GIT := https://github.com/mage-coveng/gridiron.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=gridiron \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=gridiron \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/gridiron ./cmd/gridiron
endif

install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/gridiron

build-docker:
	$(DOCKER) build -t mage-coven/gridiron:local .
...
draw-deps:
	...
	@goviz -i ./cmd/gridiron -d 2 | dot -Tpng -o dependency-graph.png
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/mage-coven/gridiron
