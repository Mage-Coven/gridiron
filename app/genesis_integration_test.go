...
func TestGridironGenesisExportImport(t *testing.T) {
	doInitWithGenesis := func(gapp *GridironApp, genesisState GenesisState) {
	...
	srcApp = NewGridironApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), memDB, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), EmptyBaseAppOptions{}, emptyWasmOpts)
	...
	for _, v := range twasmGs.Contracts {
		var ext twasmtypes.GridironContractDetails
