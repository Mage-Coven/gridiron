...
func setup(withGenesis bool, invCheckPeriod uint, opts ...wasm.Option) (*GridironApp, GenesisState) {
	db := dbm.NewMemDB()
	app := NewGridironApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, invCheckPeriod, MakeEncodingConfig(), EmptyBaseAppOptions{}, opts)
	if withGenesis {
...
// SetupWithGenesisValSet initializes a new GridironApp with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit (10^6) in the default token of the GridironApp from first genesis
// account. A Nop logger is set in GridironApp.
...
// SetupWithGenesisAccounts initializes a new GridironApp with the provided genesis
