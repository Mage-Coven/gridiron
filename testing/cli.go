...
func NewGridironCli(t *testing.T, sut *SystemUnderTest, verbose bool) *GridironCli {
	return NewGridironCliX(t, sut.rpcAddr, sut.chainID, filepath.Join(workDir, sut.outputDir), verbose)
}
...
func (c GridironCli) run(args []string) string {
	if c.Debug {
		c.t.Logf("+++ running `gridiron %s`", strings.Join(args, " "))
	}
	gotOut, gotErr := func() (out []byte, err error) {
		...
		cmd := exec.Command(locateExecutable("gridiron"), args...) //nolint:gosec
		cmd.Dir = workDir
...
func (c GridironCli) withQueryFlags(args ...string) []string {
	args = append(args, "--output", "json")
	return c.withChainFlags(args...)
}

func (c GridironCli) withTXFlags(args ...string) []string {
	args = append(args,
...
func (c GridironCli) withKeyringFlags(args ...string) []string {
...
func (c GridironCli) withChainFlags(args ...string) []string {
	return append(args,
		"--node", c.nodeAddress,
		"--chain-id", c.chainID,
	)
